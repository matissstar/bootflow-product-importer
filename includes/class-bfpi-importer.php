...
class Bfpi_Importer {
    ...
    public function __construct() {
        $this->processor = new Bfpi_Processor();
        $this->xml_parser = new Bfpi_XML_Parser();
        $this->csv_parser = new Bfpi_CSV_Parser();
    }
    ...
    public function start_import($import_data) {
        ...
        try {
            ...
            $import_record = array(
                ...
                'field_mappings' => wp_json_encode($import_data['field_mapping'] ?? array()),
                'processing_modes' => wp_json_encode($import_data['processing_modes'] ?? array()),
                'processing_configs' => wp_json_encode($import_data['processing_configs'] ?? array()),
                'ai_settings' => wp_json_encode($import_data['ai_settings'] ?? array()),
                'custom_fields' => wp_json_encode($import_data['custom_fields'] ?? array()),
            ...
            if ($is_update) {
                ...
                if ($result === false) {
                    ...
                    throw new Exception(esc_html__('Failed to update import record: ', 'bootflow-product-xml-csv-importer') . $error);
                }
                ...
                if ($result === false) {
                    $error = $wpdb->last_error;
                    throw new Exception(esc_html__('Failed to create import record: ', 'bootflow-product-xml-csv-importer') . $error);
                }
            ...
            // translators: %1$s is the import name, %2$d is the number of products
            // Log import start
            // translators: %1$s is a string, %2$d is a number
            $this->log('info', sprintf(__('Import "%1$s" started with %2$d products.', 'bootflow-product-xml-csv-importer'), $import_record['name'], $total_products));
    ...
    private function load_import_config($import_id) {
        ...
        if (!$import) {
            throw new Exception(esc_html__('Import not found.', 'bootflow-product-xml-csv-importer'));
        }

        $this->import_id = $import_id;
        
        $field_mappings = json_decode($import->field_mappings, true) ?: array();
        
        // Extract custom fields from field_mappings (they are saved with '_custom_' prefix)
        $custom_fields = array();
        ...
        // Also check the dedicated custom_fields column for backwards compatibility
        $legacy_custom_fields = json_decode($import->custom_fields ?? '[]', true) ?: array();
        if (!empty($legacy_custom_fields) && empty($custom_fields)) {
            $custom_fields = $legacy_custom_fields;
        }
    ...
    public function process_import_chunk($offset, $limit, $import_id = null) {
        ...
        try {
            ...
            $killed_specific = get_transient('bfpi_import_killed_' . $this->import_id);
            $killed_global = get_transient('bfpi_import_killed_global');
            if ($killed_specific || $killed_global) {
                // Delete specific transient after detecting it
                delete_transient('bfpi_import_killed_' . $this->import_id);
                // Only delete global transient if it's for this import
                if ($killed_global && strpos($killed_global, $this->import_id . ':') === 0) {
                ...
                return array(
                    ...
                    'completed' => false,
                    'stopped' => true,
                    'message' => 'Import killed by user'
                );
            }
            
            // CRITICAL: Use transient lock to prevent parallel execution
            $lock_key = 'bfpi_import_lock_' . $this->import_id;
            $lock_time_key = 'bfpi_import_lock_time_' . $this->import_id;
            ...
            $import_start_option_key = 'bfpi_import_start_time_' . $this->import_id;
            ...
            foreach ($products as $index => $product_data) {
                ...
                $current_status = $wpdb->get_var($wpdb->prepare(
                    "SELECT status FROM {$wpdb->prefix}bfpi_imports WHERE id = %d",
                    $this->import_id
                ));
                ...
                if ($current_status === 'failed' || $current_status === 'paused') {
                    ...
                    if ($processed_count > 0) {
                        $wpdb->query($wpdb->prepare(
                            "UPDATE {$wpdb->prefix}bfpi_imports SET processed_products = processed_products + %d WHERE id = %d",
                            $processed_count,
                            $this->import_id
                        ));
                    }
            ...
            $wpdb->query($wpdb->prepare(
                "UPDATE {$wpdb->prefix}bfpi_imports SET processed_products = processed_products + %d, last_run = %s WHERE id = %d",
                $processed_count,
                current_time('mysql'),
                $this->import_id
            ));
            ...
            if ($import->processed_products >= $import->total_products) {
                ...
                // translators: %d is the number of processed products
                $this->log('info', sprintf(__('Import completed successfully. Processed %d products.', 'bootflow-product-xml-csv-importer'), $import->processed_products));
            } else {
        ...
        try {
            ...
            if (!$passes_filters) {
                ...
                if (!empty($this->config['draft_non_matching'])) {
                    ...
                    $this->log('info', sprintf(__('Product does not match filters - will be imported as Draft (SKU: %s)', 'bootflow-product-xml-csv-importer'), $sku));
                } else {
                    // Skip product if checkbox not enabled
                    // translators: %s is the product SKU
                    $this->log('info', sprintf(__('Product skipped by import filters (SKU: %s)', 'bootflow-product-xml-csv-importer'), $sku));
                    return;
                }
            } else {
                ...
                if (!empty($this->config['draft_non_matching'])) {
                    ...
                    $sku = $product_data['sku'] ?? $product_data['id'] ?? 'Unknown';
                    // translators: %s is the product SKU
                    $this->log('info', sprintf(__('Product matches filters - will be imported as Published (SKU: %s)', 'bootflow-product-xml-csv-importer'), $sku));
                }
            }
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) { error_log('[IMPORT_PROCESS] Mapped data keys: ' . wp_json_encode(array_keys($mapped_data))); }
            ...
            if (!isset($this->_cached_import_settings)) {
                $this->_cached_import_settings = $wpdb->get_row($wpdb->prepare("SELECT update_existing, skip_unchanged FROM {$wpdb->prefix}bfpi_imports WHERE id = %d", $this->import_id), ARRAY_A);
            }
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) { error_log('[IMPORT_PROCESS] Processed data keys: ' . wp_json_encode(array_keys($processed_data))); }
            ...
            if ($existing_product_id && !$update_existing) {
                ...
                throw new Exception(sprintf(esc_html__('Product with SKU "%s" already exists and update is disabled.', 'bootflow-product-xml-csv-importer'), esc_html($processed_data['sku'])));
            }
            ...
            if ($existing_product_id && $update_existing) {
                ...
                if ($skip_unchanged && !$this->has_product_data_changed($existing_product_id, $processed_data)) {
                    ...
                    $this->log('info', sprintf(__('Skipped product (unchanged): %1$s (ID: %2$d)', 'bootflow-product-xml-csv-importer'), $product_name, $existing_product_id), esc_html($processed_data['sku']));
                ...
                $product_name = !empty($processed_data['name']) ? $processed_data['name'] : 'Product';
                // translators: %1$s is the product name, %2$d is the product ID
                $this->log('info', sprintf(__('Updated product: %1$s (ID: %2$d)', 'bootflow-product-xml-csv-importer'), $product_name, $product_id), esc_html($processed_data['sku']));
            } else {
                // Store raw product data for Shipping Class Engine (variable products need variation prices)
                $this->_current_raw_product_data = $product_data;
                ...
                $product_name = !empty($processed_data['name']) ? $processed_data['name'] : 'Product';
                // translators: %1$s is the product name, %2$d is the product ID
                $this->log('info', sprintf(__('Created product: %1$s (ID: %2$d)', 'bootflow-product-xml-csv-importer'), $product_name, $product_id), esc_html($processed_data['sku']));
            }
            ...
            if ($final_product && $final_product->is_type('variable')) {
                ...
                if ($variation_count === 0) {
                    ...
                    if (!empty($children)) {
                        ...
                        // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Placeholders dynamically built for IN clause
                        $instock_count = (int) $wpdb->get_var($wpdb->prepare(
                            "SELECT COUNT(*) FROM {$wpdb->postmeta} WHERE post_id IN ({$placeholders}) AND meta_key = '_stock_status' AND meta_value = 'instock'",
            ...
            throw new Exception(sprintf(esc_html__('Error importing product: %s', 'bootflow-product-xml-csv-importer'), esc_html($e->getMessage())));
        }
    }
    ...
    private function import_csv_variable_product($grouped_data) {
        ...
        // Get update settings
        global $wpdb;
        $import_record = $wpdb->get_row($wpdb->prepare("SELECT update_existing FROM {$wpdb->prefix}bfpi_imports WHERE id = %d", $this->import_id), ARRAY_A);
        $update_existing = ($import_record && $import_record['update_existing'] === '1') ? true : false;
        ...
        // translators: %1$s is the product name, %2$d is the number of variations
        $this->log('info', sprintf(__('Imported variable product: %1$s with %2$d variations', 'bootflow-product-xml-csv-importer'), $product_name, $variation_count), $processed_data['sku'] ?? '');
        ...
        switch ($operator) {
            ...
            case 'regex_match':
                ...
                if (@preg_match($expected, '') === false) {
                    // translators: placeholder values
                    $this->log('warning', sprintf(__('Invalid regex pattern: %s', 'bootflow-product-xml-csv-importer'), $expected));
    ...
    private function map_product_fields($product_data) {
        ...
        foreach ($field_mappings as $wc_field => $mapping_config) {
            ...
            if ($wc_field === 'sku' && isset($mapping_config['sku_mode']) && $mapping_config['sku_mode'] === 'generate') {
                ...
                $generated_sku = str_replace('{random}', strtoupper(substr(md5(uniqid(wp_rand(), true)), 0, 6)), $generated_sku);
        ...
        foreach ($mapped_data as $field_key => $value) {
            ...
            if (isset($field_mappings[$field_key])) {
                ...
                if ($is_existing_product && isset($config['update_on_sync']) && $config['update_on_sync'] !== '1' && $config['update_on_sync'] !== 1 && $config['update_on_sync'] !== true) {
                    ...
                    if ($processing_mode === 'ai_processing' || $processing_mode === 'php_formula') {                        // Just pass the raw value - it won't be saved anyway
    ...
    private function validate_product_data($product_data, $existing_product_id = null) {
        ...
        if (!empty($product_data['sku']) && !preg_match('/^[a-zA-Z0-9\-_.]+$/', $product_data['sku'])) {
            // translators: placeholder values
            throw new Exception(sprintf(esc_html__('Invalid SKU format: %s', 'bootflow-product-xml-csv-importer'), esc_html($product_data['sku'])));
        }
        ...
        if (isset($product_data['regular_price']) && $product_data['regular_price'] !== '' && $product_data['regular_price'] !== null && !is_numeric($product_data['regular_price'])) {
            // translators: placeholder values
            throw new Exception(sprintf(esc_html__('Invalid regular price: %s', 'bootflow-product-xml-csv-importer'), esc_html($product_data['regular_price'])));
        }

        if (isset($product_data['sale_price']) && !empty($product_data['sale_price']) && !is_numeric($product_data['sale_price'])) {
            // translators: placeholder values
            throw new Exception(sprintf(esc_html__('Invalid sale price: %s', 'bootflow-product-xml-csv-importer'), esc_html($product_data['sale_price'])));
        }
    }
    ...
    private function create_product($product_data, $mapped_data = array()) {
        ...
        if (empty($product_type) || $product_type === 'simple') {
            ...
            if (!empty($product_data['grouped_products'])) {
                ...
                // translators: placeholder values
                $this->log('info', sprintf(__('Auto-detected Grouped product type for "%s" (has grouped_products field)', 'bootflow-product-xml-csv-importer'), $product_data['name'] ?? $product_data['sku'] ?? 'Unknown'));
            }
            // If external_url is set, it's an External product
            elseif (!empty($product_data['external_url'])) {
                $product_type = 'external';
                // translators: placeholder values
                $this->log('info', sprintf(__('Auto-detected External product type for "%s" (has external_url field)', 'bootflow-product-xml-csv-importer'), $product_data['name'] ?? $product_data['sku'] ?? 'Unknown'));
            }
            // Default to simple
            else {
                $product_type = 'simple';
        ...
        if (!$product_id) {
            throw new Exception(esc_html__('Failed to create product.', 'bootflow-product-xml-csv-importer'));
        }
        ...
        if ($product_type === 'grouped' && $product instanceof WC_Product_Grouped) {
            if (!empty($product_data['grouped_products'])) {
                ...
                // translators: placeholder values
                $this->log('info', sprintf(__('Grouped product "%1$s" - stored pending children SKUs: %2$s', 'bootflow-product-xml-csv-importer'), $product_data['name'], $product_data['grouped_products']));
            }
        }
    ...
    private function should_update_field($field_key, $value = null, $is_new_product = false) {
        ...
        // Check selective update configuration:
    ...
    private function update_product($product_id, $product_data, $mapped_data = array()) {
        ...
        if (!$product) {
            // translators: placeholder values
            throw new Exception(sprintf(esc_html__('Product with ID %d not found.', 'bootflow-product-xml-csv-importer'), intval($product_id)));
        }
    ...
    private function parse_image_placeholders($template, $product_data) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) { error_log("PARSE_IMAGE_PLACEHOLDERS: Template='$template', entries=" . wp_json_encode($entries)); }
    ...
    /**
     * Download multiple images using WordPress HTTP API.
     * Uses wp_remote_get() for WordPress.org compliance.
     *
     * @since    1.0.0
     * @param    array $image_urls Array of image URLs
     * @param    int $product_id Product ID
     * @param    int $max_concurrent Maximum concurrent downloads (not used, kept for compatibility)
    ...
    private function download_images_parallel($image_urls, $product_id, $max_concurrent = 5) {
        ...
        $this->log('debug', 'Downloading ' . count($urls_to_download) . ' NEW images using WordPress HTTP API');
        
        // Download each image using WordPress HTTP API
        foreach ($urls_to_download as $index => $url) {
            try {
                ...
                $response = wp_remote_get($url, array(
                    ...
                    'redirection' => 5,
                    'sslverify' => false,
                    'user-agent' => 'Mozilla/5.0 (compatible; WooCommerce Import)',
                    'headers' => array(
                        'Accept' => 'image/*,*/*',
                    ),
                ));
                ...
                if (is_wp_error($response)) {
                    $this->log('warning', 'Failed to download image ' . ($index + 1) . ': ' . $url . ' (' . $response->get_error_message() . ')');
                    continue;
                }
                ...
                if ($http_code !== 200 || empty($body) || strlen($body) < 100) {
                    $this->log('warning', 'Failed to download image ' . ($index + 1) . ': ' . $url . ' (HTTP: ' . $http_code . ')');
                    continue;
                }
                ...
                $write_result = $wp_filesystem->put_contents($temp_file, $body, FS_CHMOD_FILE);
                
                if ($write_result === false) {
                    $this->log('warning', 'Failed to write temp file for: ' . $url);
                    continue;
                }
                ...
                if (empty($file_info['ext'])) {
                    ...
                    $file_info['ext'] = isset($ext_map[$mime_type]) ? $ext_map[$mime_type] : 'jpg';
                    $file_info['type'] = $mime_type;
                }
                
                // Prepare file array
                $file_array = array(
                    'name' => sanitize_file_name(pathinfo($url, PATHINFO_FILENAME)) . '.' . $file_info['ext'],
                    'tmp_name' => $temp_file
                );
                ...
                $this->log('debug', 'Downloaded image ' . ($index + 1) . ', attachment ID: ' . $attachment_id);
        ...
        $this->log('debug', 'Download completed: ' . count($attachment_ids) . ' images (downloaded: ' . count($urls_to_download) . ', cached: ' . $skipped_count . ')');
    ...
    private function download_and_attach_image($image_url, $product_id) {
        ...
        if (is_wp_error($temp_file)) {
            throw new Exception(esc_html($temp_file->get_error_message()));
        }
        ...
        if (!$file_info['ext']) {
            wp_delete_file($temp_file);
            throw new Exception(esc_html__('Invalid image file type.', 'bootflow-product-xml-csv-importer'));
        }
    ...
    private function process_custom_fields($product_id, $product_data) {
        ...
        foreach ($custom_fields as $field_config) {
            ...
            // Use sanitize_text_field instead of sanitize_key to preserve Unicode characters (like "Tesīt")
            // sanitize_key() converts to lowercase and removes non-ASCII characters
            $field_name = sanitize_text_field($field_config['name']);
            ...
            if ($value !== null) {
                ...
                // Save as meta - use field name directly (no underscore prefix) so it shows in Custom Fields
                // Note: Fields starting with _ are hidden in WordPress admin
    ...
    private function parse_downloadable_files($downloads) {
        ...
        foreach ($files as $index => $file) {
            ...
            if (strpos($file, '|') !== false) {
                ...
                $path_parts = wp_parse_url($url, PHP_URL_PATH);
                if ($path_parts) {
    ...
    private function schedule_background_processing() {
        wp_schedule_single_event(time(), 'bfpi_process_chunk', array($this->import_id, 0, 5));
    }
    ...
    private function schedule_next_chunk($offset, $limit) {
        ...
        if ($offset > 0) {
            $offset0_scheduled = wp_next_scheduled('bfpi_process_chunk', array($this->import_id, 0, $limit));
            if ($offset0_scheduled) {
                wp_unschedule_event($offset0_scheduled, 'bfpi_process_chunk', array($this->import_id, 0, $limit));
            }
        }
        
        // Schedule immediately (time() instead of time()+5) for faster processing
        $result = wp_schedule_single_event(time(), 'bfpi_process_chunk', array($this->import_id, $offset, $limit));
    }
    ...
    private function resolve_product_relationships() {
        ...
        $this->log('info', __('Starting Phase 2: Resolving product relationships...', 'bootflow-product-xml-csv-importer'));
        ...
        foreach ($grouped_products as $grouped) {
            try {
                ...
                if (!empty($child_ids)) {
                    ...
                    // translators: placeholder values
                    $this->log('info', sprintf(__('Resolved grouped product ID %1$d with %2$d children', 'bootflow-product-xml-csv-importer'), $grouped->ID, count($child_ids)));
                    $resolved_count++;
                } else {
                    // translators: placeholder values
                    $this->log('warning', sprintf(__('Could not resolve children for grouped product ID %1$d (SKUs: %2$s)', 'bootflow-product-xml-csv-importer'), $grouped->ID, $sku_list));
                    $error_count++;
                }
            } catch (Exception $e) {
                // translators: placeholder values
                $this->log('error', sprintf(__('Error resolving grouped product ID %1$d: %2$s', 'bootflow-product-xml-csv-importer'), $grouped->ID, $e->getMessage()));
                $error_count++;
            }
        }
        ...
        // translators: placeholder values
        $this->log('info', sprintf(__('Phase 2 completed: %1$d relationships resolved, %2$d errors', 'bootflow-product-xml-csv-importer'), $resolved_count, $error_count));
    }
    ...
    public function sync_variable_products_stock_status() {
        ...
        $this->log('info', __('Starting Phase 3: Syncing variable products stock status...', 'bootflow-product-xml-csv-importer'));
        ...
        // translators: placeholder values
        $this->log('info', sprintf(__('Phase 3 completed: %d variable products stock status synced', 'bootflow-product-xml-csv-importer'), $synced_count));
    }
    ...
    private function resolve_skus_to_ids($sku_list) {
        ...
        foreach ($skus as $sku) {
            ...
            if ($product_id) {
                ...
                // translators: placeholder values
                $this->log('warning', sprintf(__('Could not find product with SKU: %s', 'bootflow-product-xml-csv-importer'), $sku));
            }
        }
    ...
    private function handle_missing_products() {
        ...
        // translators: placeholder values
        $this->log('info', sprintf(__('Processing missing products with action: %s', 'bootflow-product-xml-csv-importer'), $action));
        ...
        if ($count === 0) {
            $this->log('info', __('No missing products found.', 'bootflow-product-xml-csv-importer'));
            return;
        }
            ...
            try {
                ...
                switch ($action) {
                    case 'draft':
                        ...
                        // translators: placeholder values
                        $this->log('info', sprintf(__('Product "%1$s" (SKU: %2$s) moved to draft - no longer in feed.', 'bootflow-product-xml-csv-importer'), $product->post_title, $sku), $sku);
                        break;
                    ...
                    case 'outofstock':
                        ...
                        $wc_product->save();
                        // translators: placeholder values
                        $this->log('info', sprintf(__('Product "%1$s" (SKU: %2$s) marked as out of stock - no longer in feed.', 'bootflow-product-xml-csv-importer'), $product->post_title, $sku), $sku);
                        break;
                    ...
                    case 'backorder':
                        ...
                        $wc_product->save();
                        // translators: placeholder values
                        $this->log('info', sprintf(__('Product "%1$s" (SKU: %2$s) set to backorder - no longer in feed.', 'bootflow-product-xml-csv-importer'), $product->post_title, $sku), $sku);
                        break;
                    ...
                    case 'trash':
                        ...
                        // translators: placeholder values
                        $this->log('info', sprintf(__('Product "%1$s" (SKU: %2$s) moved to trash - no longer in feed.', 'bootflow-product-xml-csv-importer'), $product->post_title, $sku), $sku);
                        break;
                    ...
                    case 'delete':
                        ...
                        // translators: placeholder values
                        $this->log('warning', sprintf(__('Product "%1$s" (SKU: %2$s) permanently deleted - no longer in feed.', 'bootflow-product-xml-csv-importer'), $product->post_title, $sku), $sku);
                        break;
                }
                ...
                $errors++;
                // translators: placeholder values
                $this->log('error', sprintf(__('Error processing missing product ID %1$d: %2$s', 'bootflow-product-xml-csv-importer'), $product->ID, $e->getMessage()));
            }
        }
        
        // translators: placeholder values
        $this->log('info', sprintf(__('Missing products cleanup completed: %1$d processed, %2$d errors.', 'bootflow-product-xml-csv-importer'), $processed, $errors));
    ...
    private function validate_import_data($import_data) {
        ...
        if (empty($import_data['file_path']) || !file_exists($import_data['file_path'])) {
            throw new Exception(esc_html__('Valid file path is required.', 'bootflow-product-xml-csv-importer'));
        }

        if (!in_array($import_data['file_type'], array('xml', 'csv'))) {
            throw new Exception(esc_html__('File type must be XML or CSV.', 'bootflow-product-xml-csv-importer'));
        }
    }
    ...
    private function log_debug($message, $context = array()) {
        ...
        $context_str = !empty($context) ? ' | Context: ' . wp_json_encode($context, JSON_UNESCAPED_UNICODE) : '';
        ...
        if (isset($this->import_id) && $this->import_id > 0) {
            $table_name = $wpdb->prefix . 'bfpi_import_logs';
