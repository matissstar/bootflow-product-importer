...
class Bfpi_AI_Providers {
    ...
    public function __construct() {
        $this->settings = get_option('bfpi_ai_settings', array());
    }
        ...
        try {
            ...
            switch ($provider) {
                ...
                default:
                    ...
                    throw new Exception(sprintf(__('Unknown AI provider: %s', 'bootflow-product-xml-csv-importer'), $provider));
            }
    ...
    private function process_with_openai($prompt, $config = array()) {
        ...
        if (empty($api_key)) {
            throw new Exception(__('OpenAI API key not configured.', 'bootflow-product-xml-csv-importer'));
        }
        ...
        if (is_wp_error($response)) {
            $error_msg = $response->get_error_message();            throw new Exception('OpenAI API request failed: ' . $error_msg);
        }
    ...
    private function process_with_gemini($prompt, $config = array()) {
        ...
        if (empty($api_key)) {
            throw new Exception(__('Google Gemini API key not configured.', 'bootflow-product-xml-csv-importer'));
        }
    ...
    private function process_with_claude($prompt, $config = array()) {
        ...
        if (empty($api_key)) {
            throw new Exception(__('Anthropic Claude API key not configured.', 'bootflow-product-xml-csv-importer'));
        }
    ...
    private function process_with_grok($prompt, $config = array()) {
        ...
        if (empty($api_key)) {
            throw new Exception(__('xAI Grok API key not configured.', 'bootflow-product-xml-csv-importer'));
        }
    ...
    private function process_with_copilot($prompt, $config = array()) {
        ...
        if (empty($api_key) || empty($endpoint)) {
            throw new Exception(__('Microsoft Copilot API key or endpoint not configured.', 'bootflow-product-xml-csv-importer'));
        }
    ...
    public function test_provider($provider, $test_prompt = '', $test_value = 'test product') {
        try {
            ...
            return array(
                ...
                'result' => $result,
                // translators: placeholder values
                'message' => sprintf(__('%s API connection successful.', 'bootflow-product-xml-csv-importer'), ucfirst($provider))
            );
            ...
            return array(
                ...
                'result' => '',
                // translators: placeholder values
                'message' => sprintf(__('%s API test failed: %s', 'bootflow-product-xml-csv-importer'), ucfirst($provider), $e->getMessage())
            );
        }
    }
...
" . wp_json_encode($source_fields, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT) . "

WOOCOMMERCE TARGET FIELDS:
" . wp_json_encode($wc_fields, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT) . "
...
- dsheet_lv, dsheet_en, dsheet_ru, dsheet_lt, dsheet_et → meta:datasheet";
        ...
        if (!empty($sample_data)) {
            ...
            if (is_array($sample_data) && isset($sample_data[0]) && is_array($sample_data[0])) {
                ...
                $prompt .= "\n\nSample values from first product:\n" . wp_json_encode($sample_data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
            }
        }
        ...
        try {
            ...
            if (!is_array($parsed) || !isset($parsed['mappings'])) {
                ...
                if (is_array($simple_parsed) && count($simple_parsed) > 0) {
                    ...
                    // Log detailed error
                    $error_msg = 'JSON parse error: ' . json_last_error_msg();                    throw new Exception('Failed to parse AI response. ' . $error_msg . ' Response: ' . substr($cleaned_result, 0, 300));
                }
            }
