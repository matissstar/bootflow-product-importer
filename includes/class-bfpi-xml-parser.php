...
class Bfpi_XML_Parser {
    ...
    public function validate_xml_file($file_path) {
        try {
            ...
            $max_size = get_option('bfpi_settings', array())['max_file_size'] ?? 100;
            ...
            return array(
                'valid' => true,
                'message' => __('XML file is valid.', 'bootflow-product-xml-csv-importer'),
                'file_size' => $file_size
            );
    ...
    public function count_products_and_extract_structure($file_path, $product_wrapper) {
        try {
            // Set time and memory limits for large files
            if (function_exists('set_time_limit')) { @set_time_limit(600); } // 10 minutes for counting
            wp_raise_memory_limit('admin');
