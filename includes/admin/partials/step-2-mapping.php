...
$can_ai_mapping = Bfpi_Features::is_available('ai_auto_mapping');
$can_templates = Bfpi_Features::is_available('mapping_templates');
...
$can_php_processing = Bfpi_Features::is_available('php_processing');
$can_ai_processing = Bfpi_Features::is_available('ai_processing');
$can_hybrid_processing = Bfpi_Features::is_available('hybrid_processing');
$can_advanced_formulas = Bfpi_Features::is_available('advanced_formulas');
        ...
        <div class="wc-ai-import-main">
            ...
            <div class="wc-ai-import-card" style="margin-bottom: 20px;">
                ...
                <table class="form-table">
                    ...
                    <tr>
                        <th scope="row">
                            <?php esc_html_e('Scheduled Import', 'bootflow-product-xml-csv-importer'); ?>
                        </th>
                        <td>
                            <select name="schedule_type" id="schedule_type" class="regular-text">
                                <option value="disabled" <?php selected($schedule_type, 'disabled'); ?>><?php esc_html_e('Manual Import Only', 'bootflow-product-xml-csv-importer'); ?></option>
                                <option value="15min" <?php selected($schedule_type, '15min'); ?>><?php esc_html_e('Every 15 Minutes', 'bootflow-product-xml-csv-importer'); ?></option>
                                <option value="hourly" <?php selected($schedule_type, 'hourly'); ?>><?php esc_html_e('Every Hour', 'bootflow-product-xml-csv-importer'); ?></option>
                                <option value="6hours" <?php selected($schedule_type, '6hours'); ?>><?php esc_html_e('Every 6 Hours', 'bootflow-product-xml-csv-importer'); ?></option>
                                <option value="daily" <?php selected($schedule_type, 'daily'); ?>><?php esc_html_e('Daily Import', 'bootflow-product-xml-csv-importer'); ?></option>
                                <option value="weekly" <?php selected($schedule_type, 'weekly'); ?>><?php esc_html_e('Weekly Import', 'bootflow-product-xml-csv-importer'); ?></option>
                                <option value="monthly" <?php selected($schedule_type, 'monthly'); ?>><?php esc_html_e('Monthly Import', 'bootflow-product-xml-csv-importer'); ?></option>
                            </select>
                            <p class="description">
                                <?php esc_html_e('Automatically re-import from URL on a schedule. Requires server cron to be configured.', 'bootflow-product-xml-csv-importer'); ?>
                                <a href="<?php echo esc_url(admin_url('admin.php?page=bfpi-import-settings&tab=scheduling')); ?>" target="_blank"><?php esc_html_e('View cron setup instructions', 'bootflow-product-xml-csv-importer'); ?> →</a>
                            </p>
                        </td>
                    </tr>
                    
                    <!-- Schedule Method (shown when schedule is enabled) -->
                    <tr id="schedule_method_row_new" style="<?php echo esc_attr( ($schedule_type === 'disabled' || empty($schedule_type)) ? 'display:none;' : '' ); ?>">
                        ...
                        <td>
                            ...
                            $cron_url_new = admin_url('admin-ajax.php') . '?action=bfpi_cron&secret=' . $cron_secret;
                            ?>
                            <fieldset>
                                <label style="display: block; margin-bottom: 10px; padding: 10px; border: 2px solid <?php echo esc_attr($schedule_method === 'action_scheduler' ? '#0073aa' : '#ddd'); ?>; border-radius: 6px; cursor: pointer; background: <?php echo esc_attr($schedule_method === 'action_scheduler' ? '#f0f6fc' : '#fff'); ?>;">
                                    ...
                                    <p class="description" style="margin: 5px 0 0 20px;">
                                        <?php esc_html_e('Automatically continues until complete. No server cron needed.', 'bootflow-product-xml-csv-importer'); ?>
                                    </p>
                                </label>
                                ...
                                <label style="display: block; padding: 10px; border: 2px solid <?php echo esc_attr($schedule_method === 'server_cron' ? '#0073aa' : '#ddd'); ?>; border-radius: 6px; cursor: pointer; background: <?php echo esc_attr($schedule_method === 'server_cron' ? '#f0f6fc' : '#fff'); ?>;">
                                    ...
                                    <p class="description" style="margin: 5px 0 0 20px;">
                                        <?php esc_html_e('Processes entire import in one request. Requires server cron setup.', 'bootflow-product-xml-csv-importer'); ?>
                                    </p>
                                </label>
                            </fieldset>
                            ...
                            <div id="server_cron_url_new" style="<?php echo esc_attr($schedule_method !== 'server_cron' ? 'display:none;' : ''); ?> margin-top: 10px; padding: 10px; background: #f8f9fa; border-radius: 4px;">
                                <strong><?php esc_html_e('Cron URL:', 'bootflow-product-xml-csv-importer'); ?></strong>
                                ...
                                <button type="button" class="button button-small" style="margin-top: 5px;" onclick="navigator.clipboard.writeText('<?php echo esc_js($cron_url_new); ?>'); alert('Copied!');">
                                    <?php esc_html_e('Copy URL', 'bootflow-product-xml-csv-importer'); ?>
                                </button>
                            </div>
                        </td>
                    </tr>
                ...
                <form id="wc-ai-import-mapping-form" method="post">
                    ...
                    <div class="mapping-recipes-section" style="background: #f8f9fa; border: 1px solid #e2e4e7; border-radius: 6px; padding: 15px; margin-bottom: 20px;">
                        <h4 style="margin: 0 0 12px 0; font-size: 14px; color: #1e1e1e;">
                            ...
                            <?php esc_html_e('Mapping Templates', 'bootflow-product-xml-csv-importer'); ?>
                        </h4>
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 10px; align-items: flex-end;">
                            ...
                            <div style="flex: 1; min-width: 200px;">
                                <label style="display: block; font-size: 11px; color: #666; margin-bottom: 4px;">
                                    <?php esc_html_e('Save current mapping as template:', 'bootflow-product-xml-csv-importer'); ?>
                                </label>
                                <div style="display: flex; gap: 5px;">
                                    <input type="text" 
                                           id="recipe-name-input" 
                                           placeholder="<?php esc_attr_e('Enter template name...', 'bootflow-product-xml-csv-importer'); ?>"
                                           style="flex: 1; min-width: 150px; height: 32px;">
                                    <button type="button" class="button" id="save-recipe-btn" title="<?php esc_attr_e('Save Template', 'bootflow-product-xml-csv-importer'); ?>">
                            ...
                            <div style="flex: 1; min-width: 200px;">
                                <label style="display: block; font-size: 11px; color: #666; margin-bottom: 4px;">
                                    <?php esc_html_e('Load saved template:', 'bootflow-product-xml-csv-importer'); ?>
                                </label>
                                <div style="display: flex; gap: 5px;">
                                    <select id="recipe-select" style="flex: 1; min-width: 150px; height: 32px;">
                                        <option value=""><?php esc_html_e('-- Select template --', 'bootflow-product-xml-csv-importer'); ?></option>
                                    </select>
                                    <button type="button" class="button" id="load-recipe-btn" title="<?php esc_attr_e('Load Template', 'bootflow-product-xml-csv-importer'); ?>">
                                        <span class="dashicons dashicons-upload" style="margin-top: 3px;"></span>
                                        <?php esc_html_e('Load', 'bootflow-product-xml-csv-importer'); ?>
                                    </button>
                                    <button type="button" class="button" id="delete-recipe-btn" title="<?php esc_attr_e('Delete Template', 'bootflow-product-xml-csv-importer'); ?>" style="color: #a00;">
                    ...
                    <div class="smart-mapping-options" style="background: #f8f9fa; border: 1px solid #e9ecef; border-radius: 6px; padding: 15px 20px; margin-bottom: 20px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
                            <div>
                                <h4 style="margin: 0 0 5px 0; font-size: 14px; color: #495057; font-weight: 600;">
                                    <?php esc_html_e('AI Auto-Mapping', 'bootflow-product-xml-csv-importer'); ?>
                                </h4>
                                <p style="margin: 0; font-size: 13px; color: #6c757d;">
                                    <?php esc_html_e('Let AI automatically match your file columns to WooCommerce fields.', 'bootflow-product-xml-csv-importer'); ?>
                                </p>
                            </div>
                            ...
                            <div style="display: flex; gap: 8px; align-items: center;">
                                ...
                                <button type="button" id="btn-ai-auto-map" class="button button-primary" style="display: flex; align-items: center; gap: 5px;">
                                    ...
                                    <?php esc_html_e('AI Auto-Map', 'bootflow-product-xml-csv-importer'); ?>
                                </button>
                                ...
                                <span style="color: #6c757d; font-size: 12px;">
                                    <?php esc_html_e('Configure AI provider in Settings to enable auto-mapping', 'bootflow-product-xml-csv-importer'); ?>
                                </span>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <div id="ai-mapping-status" style="display: none; margin-top: 12px; padding: 10px; background: #e9ecef; border-radius: 4px; font-size: 13px;">
                            <div id="ai-mapping-progress" style="display: none;">
                                ...
                                <span id="ai-mapping-progress-text"><?php esc_html_e('AI analyzing fields...', 'bootflow-product-xml-csv-importer'); ?></span>
                            </div>
                                        ...
                                        <div class="field-mapping-row" data-field="<?php echo esc_attr($field_key); ?>" data-field-type="<?php echo esc_attr($field['type']); ?>">
                                            ...
                                            <div class="processing-mode">
                                                <select name="field_mapping[<?php echo esc_attr($field_key); ?>][processing_mode]" class="processing-mode-select">
                                                    <option value="direct"><?php esc_html_e('Direct Mapping', 'bootflow-product-xml-csv-importer'); ?></option>
                                                    ...
                                                    <option value="hybrid"><?php esc_html_e('Hybrid (PHP + AI)', 'bootflow-product-xml-csv-importer'); ?></option>
                                                </select>
                                            </div>
                                                ...
                                                <div class="config-content">
                                                    ...
                                                    <div class="php-formula-config config-panel">
                                                        ...
                                                        <textarea name="field_mapping[<?php echo esc_attr($field_key); ?>][php_formula]" 
                                                                  ...
                                                                  rows="3"></textarea>
                                                        <p class="description">
                                                            <?php esc_html_e('✅ Simple formulas work directly: <code>$value * 1.2</code>', 'bootflow-product-xml-csv-importer'); ?><br>
                                                            <?php esc_html_e('✅ Complex formulas need return: <code>if ($value > 100) { return $value * 0.9; } return $value;</code>', 'bootflow-product-xml-csv-importer'); ?><br>
                                                            <?php esc_html_e('✅ Access any XML field: <code>$value . " - " . $brand</code>', 'bootflow-product-xml-csv-importer'); ?>
                                                        </p>
                                                    ...
                                                    <div class="hybrid-config config-panel">
                                                        <label><?php esc_html_e('AI Prompt (executed first):', 'bootflow-product-xml-csv-importer'); ?></label>
                                                        <textarea name="field_mapping[<?php echo esc_attr($field_key); ?>][hybrid_ai_prompt]" 
                                                                  placeholder="<?php esc_html_e('e.g., Enhance this text for better readability', 'bootflow-product-xml-csv-importer'); ?>" 
                                                                  rows="2"></textarea>
                                                        ...
                                                        <p class="description">
                                                            ...
                                                            <?php esc_html_e('AI result will be passed to PHP formula', 'bootflow-product-xml-csv-importer'); ?>
                                                        </p>
                                                        
                                                        <label style="margin-top: 15px; display: block;"><?php esc_html_e('PHP Formula (applied to AI result):', 'bootflow-product-xml-csv-importer'); ?></label>
                                                        ...
                                                        <p class="description">
                                                            <strong><?php esc_html_e('Use $value as input', 'bootflow-product-xml-csv-importer'); ?></strong> - 
                                                            <?php esc_html_e('Final technical adjustments (trim, lowercase, etc)', 'bootflow-product-xml-csv-importer'); ?>
                                                        <p class="description">
                                                            <strong><?php esc_html_e('Use $value as input', 'bootflow-product-xml-csv-importer'); ?></strong> - 
                                                            <?php esc_html_e('Final technical adjustments (trim, lowercase, etc)', 'bootflow-product-xml-csv-importer'); ?>
                                                        </p>
            ...
            <div class="config-content">
                ...
                <div class="php-formula-config config-panel" style="display: none;">
                    ...
                    <textarea name="custom_fields[{index}][php_formula]" 
                              placeholder="<?php esc_html_e('e.g., return $product[\'price\'][\'@attributes\'][\'currency\'] ?? \'USD\';', 'bootflow-product-xml-csv-importer'); ?>" 
                              rows="3"></textarea>
                    <p class="description">
                        <?php esc_html_e('Access product data: <code>$product[\'field\']</code> or <code>$value</code> for source field', 'bootflow-product-xml-csv-importer'); ?>
                    </p>
                </div>
                ...
                <div class="ai-processing-config config-panel" style="display: none;">
                    ...
                    <textarea name="custom_fields[{index}][ai_prompt]" 
                              placeholder="<?php esc_html_e('e.g., Extract the currency code from this value', 'bootflow-product-xml-csv-importer'); ?>" 
                              rows="3"></textarea>
                    <div style="margin-top: 8px;">
                ...
                <div class="php-formula-config config-panel" style="display: none;">
                    <label><?php esc_html_e('PHP Formula:', 'bootflow-product-xml-csv-importer'); ?></label>
                    <textarea name="custom_fields[{index}][php_formula]" 
                              placeholder="<?php esc_html_e('e.g., return $value * 1.21;', 'bootflow-product-xml-csv-importer'); ?>" 
                              rows="3"></textarea>
                    <p class="description"><?php esc_html_e('Use $value for source field value, $product for full product data', 'bootflow-product-xml-csv-importer'); ?></p>
                    <div style="margin-top: 8px;">
                        <button type="button" class="button button-small test-php-field" data-field="custom-{index}">
                            <?php esc_html_e('Test PHP', 'bootflow-product-xml-csv-importer'); ?>
                        </button>
                        <span class="test-result" style="margin-left: 10px; font-style: italic;"></span>
                    </div>
                </div>
                ...
                <div class="hybrid-config config-panel" style="display: none;">
                    ...
                    <div style="margin-top: 8px;">
                        <button type="button" class="button button-small test-ai-field" data-field="custom-{index}">
                            <?php esc_html_e('Test Hybrid', 'bootflow-product-xml-csv-importer'); ?>
                        </button>
                        <span class="test-result" style="margin-left: 10px; font-style: italic;"></span>
                    </div>
                </div>
            ...
            <div style="flex: 1;">
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <div style="display: flex; gap: 10px; align-items: start; flex-wrap: wrap;">
                        ...
                        <select name="attributes[{{index}}][values_processing_mode]" class="processing-mode-select attribute-values-processing" style="width: 150px;">
                            ...
                            <option value="php_formula"><?php esc_html_e('PHP Formula', 'bootflow-product-xml-csv-importer'); ?></option>
                            <option value="ai_processing"><?php esc_html_e('AI Processing', 'bootflow-product-xml-csv-importer'); ?></option>
                            <option value="hybrid"><?php esc_html_e('Hybrid', 'bootflow-product-xml-csv-importer'); ?></option>
                        </select>
                    </div>
                    <div class="attribute-values-config" style="margin-top: 10px; display: none;">
                        <div class="php-formula-config config-panel" style="display: none;">
                            <label style="font-size: 12px; font-weight: 600;"><?php esc_html_e('PHP Formula:', 'bootflow-product-xml-csv-importer'); ?></label>
                            <textarea name="attributes[{{index}}][values_php_formula]" placeholder="<?php esc_html_e('e.g., explode(\",\", $value)', 'bootflow-product-xml-csv-importer'); ?>" rows="2" style="width: 100%; max-width: 500px;"></textarea>
                        </div>
                        <div class="ai-prompt-config config-panel" style="display: none;">
                            <label style="font-size: 12px; font-weight: 600;"><?php esc_html_e('AI Prompt:', 'bootflow-product-xml-csv-importer'); ?></label>
                            <textarea name="attributes[{{index}}][values_ai_prompt]" placeholder="<?php esc_html_e('e.g., Extract color names from this text', 'bootflow-product-xml-csv-importer'); ?>" rows="2" style="width: 100%; max-width: 500px;"></textarea>
                        </div>
                        <div class="hybrid-config config-panel" style="display: none;">
                            <label style="font-size: 12px; font-weight: 600;"><?php esc_html_e('PHP Formula (runs first):', 'bootflow-product-xml-csv-importer'); ?></label>
                            <textarea name="attributes[{{index}}][values_hybrid_php]" placeholder="<?php esc_html_e('Pre-process with PHP', 'bootflow-product-xml-csv-importer'); ?>" rows="2" style="width: 100%; max-width: 500px;"></textarea>
                            <label style="font-size: 12px; font-weight: 600; margin-top: 8px;"><?php esc_html_e('AI Prompt (runs after):', 'bootflow-product-xml-csv-importer'); ?></label>
                            <textarea name="attributes[{{index}}][values_hybrid_ai]" placeholder="<?php esc_html_e('Then process with AI', 'bootflow-product-xml-csv-importer'); ?>" rows="2" style="width: 100%; max-width: 500px;"></textarea>
                        </div>
                    </div>
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <div class="attribute-image-config" style="margin-top: 10px; display: none;">
                        <div class="php-formula-config config-panel" style="display: none;">
                            <label style="font-size: 12px; font-weight: 600;"><?php esc_html_e('PHP Formula:', 'bootflow-product-xml-csv-importer'); ?></label>
                            <textarea name="attributes[{{index}}][image_php_formula]" placeholder="<?php esc_html_e('e.g., str_replace(\"small\", \"large\", $value)', 'bootflow-product-xml-csv-importer'); ?>" rows="2" style="width: 100%; max-width: 500px;"></textarea>
                        </div>
                        <div class="ai-prompt-config config-panel" style="display: none;">
                            <label style="font-size: 12px; font-weight: 600;"><?php esc_html_e('AI Prompt:', 'bootflow-product-xml-csv-importer'); ?></label>
                            <textarea name="attributes[{{index}}][image_ai_prompt]" placeholder="<?php esc_html_e('e.g., Extract image URL from HTML', 'bootflow-product-xml-csv-importer'); ?>" rows="2" style="width: 100%; max-width: 500px;"></textarea>
                        </div>
                        <div class="hybrid-config config-panel" style="display: none;">
                            <label style="font-size: 12px; font-weight: 600;"><?php esc_html_e('PHP Formula (runs first):', 'bootflow-product-xml-csv-importer'); ?></label>
                            <textarea name="attributes[{{index}}][image_hybrid_php]" placeholder="<?php esc_html_e('Pre-process with PHP', 'bootflow-product-xml-csv-importer'); ?>" rows="2" style="width: 100%; max-width: 500px;"></textarea>
                            <label style="font-size: 12px; font-weight: 600; margin-top: 8px;"><?php esc_html_e('AI Prompt (runs after):', 'bootflow-product-xml-csv-importer'); ?></label>
                            <textarea name="attributes[{{index}}][image_hybrid_ai]" placeholder="<?php esc_html_e('Then process with AI', 'bootflow-product-xml-csv-importer'); ?>" rows="2" style="width: 100%; max-width: 500px;"></textarea>
                        </div>
                    </div>
                        ...
                        <div class="var-price-processing-config" style="display: none; margin-top: 10px; padding: 10px; background: #f9f9f9; border-radius: 4px;">
                            ...
                            <div class="var-price-php-config config-panel" style="display: none;">
                                <textarea name="attributes[{{index}}][var_price_php_formula]" 
                                          placeholder="<?php esc_attr_e('e.g., $value * 1.21', 'bootflow-product-xml-csv-importer'); ?>" 
                                          rows="2" style="width: 100%; max-width: 500px;"></textarea>
                                <p class="description" style="font-size: 11px;"><?php esc_html_e('Variables: $value, $parent_price, $sku, etc.', 'bootflow-product-xml-csv-importer'); ?></p>
                            </div>
                            <!-- AI Processing -->
                            <div class="var-price-ai-config config-panel" style="display: none;">
                                <textarea name="attributes[{{index}}][var_price_ai_prompt]" 
                                          placeholder="<?php esc_attr_e('e.g., Convert price and add 21% VAT', 'bootflow-product-xml-csv-importer'); ?>" 
                                          rows="2" style="width: 100%; max-width: 500px;"></textarea>
                            </div>
                            <!-- Hybrid -->
                            <div class="var-price-hybrid-config config-panel" style="display: none;">
                                <div style="margin-bottom: 8px;">
                                    <label style="font-size: 11px; color: #666;"><?php esc_html_e('PHP Pre-processing:', 'bootflow-product-xml-csv-importer'); ?></label>
                                    <textarea name="attributes[{{index}}][var_price_hybrid_php]" 
                                              placeholder="<?php esc_attr_e('$value * 1.21', 'bootflow-product-xml-csv-importer'); ?>" 
                                              rows="1" style="width: 100%; max-width: 500px;"></textarea>
                                </div>
                                <div>
                                    <label style="font-size: 11px; color: #666;"><?php esc_html_e('AI Prompt:', 'bootflow-product-xml-csv-importer'); ?></label>
                                    <textarea name="attributes[{{index}}][var_price_hybrid_ai]" 
                                              placeholder="<?php esc_attr_e('Round to nearest .99', 'bootflow-product-xml-csv-importer'); ?>" 
                                              rows="1" style="width: 100%; max-width: 500px;"></textarea>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="var-description-processing-config" style="display: none; margin-top: 10px; padding: 10px; background: #f9f9f9; border-radius: 4px;">
                                    ...
                                    <div class="var-description-php-config config-panel" style="display: none;">
                                        <textarea name="attributes[{{index}}][var_description_php_formula]" 
                                                  placeholder="<?php esc_attr_e('e.g., \"Size: \" . $value . \" - \" . $parent_description', 'bootflow-product-xml-csv-importer'); ?>" 
                                                  rows="2" style="width: 100%; max-width: 500px;"></textarea>
                                        <p class="description" style="font-size: 11px;"><?php esc_html_e('Variables: $value, $parent_description, $sku, $name, etc.', 'bootflow-product-xml-csv-importer'); ?></p>
                                    </div>
                                    <!-- AI Processing -->
                                    <div class="var-description-ai-config config-panel" style="display: none;">
                                        <textarea name="attributes[{{index}}][var_description_ai_prompt]" 
                                                  placeholder="<?php esc_attr_e('e.g., Generate unique variation description', 'bootflow-product-xml-csv-importer'); ?>" 
                                                  rows="2" style="width: 100%; max-width: 500px;"></textarea>
                                    </div>
                                    <!-- Hybrid -->
                                    <div class="var-description-hybrid-config config-panel" style="display: none;">
                                        <div style="margin-bottom: 8px;">
                                            ...
                                            <textarea name="attributes[{{index}}][var_description_hybrid_php]" 
                                                      placeholder="<?php esc_attr_e('$parent_description . \" | \" . $value', 'bootflow-product-xml-csv-importer'); ?>" 
                                                      rows="1" style="width: 100%; max-width: 500px;"></textarea>
                                        </div>
                                        <div>
                                            <label style="font-size: 11px; color: #666;"><?php esc_html_e('AI Prompt:', 'bootflow-product-xml-csv-importer'); ?></label>
                                            <textarea name="attributes[{{index}}][var_description_hybrid_ai]" 
                                                      placeholder="<?php esc_attr_e('Improve and make unique', 'bootflow-product-xml-csv-importer'); ?>" 
                                                      rows="1" style="width: 100%; max-width: 500px;"></textarea>
                                        </div>
                                    </div>
                                </div>
...
ob_start();
?>
var wcAiImportData = <?php echo wp_json_encode($bfpi_step2_data); ?>;
// Copy ajax_url and nonce from localized data if not set
if (typeof bfpi_ajax !== 'undefined') {
    wcAiImportData.ajax_url = bfpi_ajax.ajax_url;
    wcAiImportData.nonce = bfpi_ajax.nonce;
}
