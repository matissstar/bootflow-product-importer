...
class Bfpi_CSV_Parser {
    ...
    public function convert_to_utf8($file_path) {
        ...
        if ($wp_filesystem->put_contents($temp_file, $utf8_content, FS_CHMOD_FILE)) {
            return $temp_file;
        }
    ...
    public function validate_csv_file($file_path) {
        try {
            ...
            return array(
                'valid' => true,
                // translators: placeholder values
                'message' => sprintf(__('CSV file is valid with %d columns.', 'bootflow-product-xml-csv-importer'), count($headers)),
                'file_size' => $file_size,
