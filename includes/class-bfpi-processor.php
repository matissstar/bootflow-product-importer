...
class Bfpi_Processor {
    ...
    public function __construct() {
        if (class_exists('Bfpi_AI_Providers')) {
            $this->ai_providers = new Bfpi_AI_Providers();
        }
    }
    ...
    public function process_field($value, $config, $product_data = array()) {
        try {
            if (defined('WP_DEBUG') && WP_DEBUG) { error_log("PROCESSOR: process_field called with value={$value}, config=" . wp_json_encode($config)); }
    ...
    private function process_ai($value, $config, $product_data = array()) {
        ...
        if ($this->ai_providers === null) {
            $this->log_debug('AI_PROCESSING_UNAVAILABLE: AI providers module not loaded', array());
            return $this->process_direct($value);
        }
        ...
        $this->log_debug('AI_PROCESSING_CONTEXT: Built product context', array(
            ...
            'context_size' => strlen(wp_json_encode($context))
        ));
    ...
    private function execute_php_formula($formula, $value, $product_data = array()) {
        ...
        try {
            ...
            if (empty($settings['enable_php_formulas'])) {
                throw new Exception(__('PHP formulas are disabled in settings.', 'bootflow-product-xml-csv-importer'));
            }
            ...
            foreach ($dangerous_patterns as $pattern) {
                if (preg_match($pattern, $formula)) {
                    throw new Exception(__('Formula contains disallowed constructs.', 'bootflow-product-xml-csv-importer'));
                }
            }
            ...
            if (preg_match_all('/\b([a-zA-Z_][a-zA-Z0-9_]*)\s*\(/i', $formula_without_strings, $matches)) {
                if (defined('WP_DEBUG') && WP_DEBUG) { error_log("Found function calls in formula: " . wp_json_encode($matches[1])); }
                foreach ($matches[1] as $function_name) {
                    if (!in_array(strtolower($function_name), $allowed_functions)) {
                        ...
                        throw new Exception(sprintf(__('Function "%s" is not allowed.', 'bootflow-product-xml-csv-importer'), $function_name));
                    }
                }
            }
            ...
            foreach ($product_data as $key => $val) {
                ...
                if (in_array($key, ['category', 'product_name', 'code', 'eans.ean', 'eans'])) {
                    $val_preview = is_array($val) ? wp_json_encode($val) : (is_string($val) ? substr($val, 0, 100) : gettype($val));
    ...
    private function safe_eval($code, $variables = array(), $value_param = null) {
        // Build variable extraction for isolated scope
        $variables['value'] = $value_param;

        // Wrap in anonymous function for sandboxing
        $var_exports = array();
        foreach ($variables as $var_name => $var_value) {
            $var_exports[] = '$' . $var_name . ' = ' . var_export($var_value, true) . ';';
        }
        $var_setup = implode("\n", $var_exports);
        ...
        try {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                if (defined('WP_DEBUG') && WP_DEBUG) { error_log('Bfpi safe_eval error: ' . $e->getMessage()); }
            }
            return $value_param;
        }
    }
    ...
    /**
     * Validate a PHP formula for safety.
     *
     * @param string $formula The formula to validate.
     * @return true|WP_Error True if safe, WP_Error with message if not.
     */
    public static function validate_formula($formula) {
        ...
        $dangerous_patterns = array(
            '/\b(eval|exec|system|shell_exec|passthru|file_get_contents|file_put_contents|fopen|fwrite|include|require)\s*\(/i',
            '/\$\w*GLOBALS|\$_[A-Z]+/i',
            '/\b(function|class|interface|trait)\s+/i',
            '/\-\>|\:\:/i',
            '/\$this\b/i',
        );
        ...
        foreach ($dangerous_patterns as $pattern) {
            if (preg_match($pattern, $formula)) {
                return new \WP_Error('dangerous_formula', __('Formula contains disallowed constructs.', 'bootflow-product-xml-csv-importer'));
            }
        }
        ...
        if (preg_match_all('/\b([a-zA-Z_][a-zA-Z0-9_]*)\s*\(/i', $formula_without_strings, $matches)) {
            foreach ($matches[1] as $function_name) {
                if (!in_array(strtolower($function_name), $allowed_functions)) {
                    return new \WP_Error(
                        'disallowed_function',
                        sprintf(__('Function "%s" is not allowed.', 'bootflow-product-xml-csv-importer'), $function_name)
                    );
                }
            }
        }
