...
class WC_XML_CSV_AI_Import_i18n {
    ...
    public function reload_textdomain_for_user() {
        ...
        if (!$admin_locale || $admin_locale === 'en_US') {
            ...
            unload_textdomain('bootflow-product-importer');
            return;
        }
        ...
        unload_textdomain('bootflow-product-importer');
        $mofile = WC_XML_CSV_AI_IMPORT_PLUGIN_DIR . 'languages/bootflow-product-importer-' . $admin_locale . '.mo';
        if (file_exists($mofile)) {
            load_textdomain('bootflow-product-importer', $mofile);
        }
    }
